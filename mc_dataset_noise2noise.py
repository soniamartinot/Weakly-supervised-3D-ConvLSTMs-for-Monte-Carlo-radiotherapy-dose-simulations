...
class MC3D_Dataset_Noise2Noise(Dataset):
    def __init__(self, train_list, n_frames, 
                 patch_size=80, all_channels=False, 
                 normalized_by_gt=True, standardize=False, 
                 uncertainty_thresh=0.02, dose_thresh=0.8, seed=1, transform=True, 
                 unet=False, verbose=False, 
                 add_ct=False, ct_norm=False,
                 high_dose_only=False, p1=0.5, p2=0.2,
                 single_frame=False,
                 depth=8, 
                 mode="infinite",
                 n_samples=15000,
        ...
        # Set attributes
        self.train_list = train_list                   # List comprising the paths to the cases used in the dataset
        self.n_frames = n_frames                       # Number of noisy frames to input the model
        self.patch_size = patch_size                   # Size of the patch
        self.all_channels = all_channels               # Whether to create patches with respect to each dimension
        self.normalized_by_gt = normalized_by_gt       # Whether to normalize the data beforehand
        self.standardize = standardize                 # Whether to standardize the data
        self.uncertainty_thresh = uncertainty_thresh   # Set the uncertainty threshold below which we select the training samples
        self.dose_thresh = dose_thresh                 # Set the dose threshold above which we select the training samples
        self.seed = seed                               # Set the random seed
        self.transform = transform                     # Whether to add basic data augmentation
        self.unet = unet                               # Whether the model is unet like
        self.add_ct = add_ct                           # Whether to add the corresponding CT slice to the model's input
        self.ct_norm = ct_norm                         # How to normalize the CT volume
        self.high_dose_only = high_dose_only
        self.p1 = p1
        self.p2 = p2
        ...
        self.dict_ct = {case_path: 
                        np.load("/workspace/workstation/numpy_cropped_images/cropped_{}.npy".format(os.path.basename(case_path)), allow_pickle=True) 
        ...
        if patch:
            ...
            if self.single_frame:
                ...
                if self.add_ct:
                    ...
                    if self.noise2noise:
                        # Preferred normalization will be the minimax or the standardization
                        candidates = np.arange(len(particles_to_path[n_particles])).tolist()
                        idx_noisy, idx_noisy_target = random.sample(candidates, 2)
                        path = particles_to_path[n_particles][idx_noisy]
                        path_noisy_target = particles_to_path[n_particles][idx_noisy_target]
                        ground_truth = np.load(path_noisy_target, allow_pickle=True)[x-half_depth:x+half_depth, 
                                                                                     y-half_patch_size:y+half_patch_size, 
                                                                                     z-half_patch_size:z+half_patch_size]
                    else:
            ...
            # If several input frames
                ...
                if self.add_ct:
                    ...
                    for i, n in enumerate(particles[:self.n_frames]):
                        ...
                        if self.noise2noise:
                            # Preferred normalization will be the minimax or the standardization
                            candidates = np.arange(len(particles_to_path[n])).tolist()
                            idx_noisy, idx_noisy_target = random.sample(candidates, 2)
                            path = particles_to_path[n][idx_noisy]
                            path_noisy_target = particles_to_path[n][idx_noisy_target]
                            if i == len(particles[:self.n_frames]) -1: 
                                ground_truth = np.load(path_noisy_target, allow_pickle=True)[x-half_depth:x+half_depth, 
                                                                                             y-half_patch_size:y+half_patch_size, 
                                                                                             z-half_patch_size:z+half_patch_size]
                        else:
